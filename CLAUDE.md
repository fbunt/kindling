...
├── config.py            # Single source of truth: CHAT_MODELS (selector allowlist), DEFAULT_CHAT_MODEL, LITE_MODEL, MAX_TOOL_ROUNDS
...
│   ├── chat.py          # GET /api/config (model list); POST /api/chat - thin SSE wrapper around chat_loop.run_chat_turn
...
- **Plot serving**: worker plots are materialized to `plots/` and served by a session-gated route (`routes/plots.py`, 401 without a keystore token, strict filename allowlist) — deliberately **not** a public static mount. `plots/` is wiped on startup and capped at 500 files (oldest pruned on each write, `sandbox/pool.py`); history refs to wiped/pruned plots degrade to text stubs (see Conversation history). Worker replies are capped at 6 MB and degrade gracefully (tabular rows halved with a truncation note, then an actionable error).
- **SSE streaming**: Chat endpoint streams events to the frontend: `status` (thinking/running_query), `rejected` (failed/blocked queries with error reason), `done` (final response, `model`, `plots` as `{url, name, path, epoch}` refs, `queries`; no image bytes), `error`. Any non-2xx / non-`text/event-stream` response (400/413/415/422) is a JSON `{detail}` the client renders as an error bubble instead of hanging on the spinner.
